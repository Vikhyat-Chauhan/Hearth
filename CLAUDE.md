...
| Mark a done occurrence undone (History tab) | P2 | `app/api/chore-logs/route.ts` (`DELETE`), `components/MarkUndoneButton.tsx`, `components/ChoreHistoryList.tsx`, `lib/chore-sync.ts` (`restoreOccurrenceOnCalendars`), `lib/calendar.ts` (`restoreChoreInstance`) | feat/mark-overdue-chores-done |
| Lock future occurrences (can't mark done before their date) | P2 | `lib/recurrence.ts` (`isFutureOccurrence`), `app/api/chore-logs/route.ts` (`POST` guard), `components/ChoreList.tsx`, `components/MarkDoneButton.tsx` (`upcoming` → 🔒 Upcoming badge) | feat/lock-future-occurrences |
...
| Scheduled maintenance crons (retention + calendar refresh/cleanup) | P2 | `lib/retention.ts` (`pruneOldData`, `RETENTION_*` env), `app/api/cron/retention/route.ts`, `lib/calendar-twoway.ts` (`refreshExpiringWatches`), `app/api/calendar/refresh-channels/route.ts` (`APP_BASE_URL`), `app/api/cron/calendar-cleanup/route.ts`, `vercel.json` (3 crons, `CRON_SECRET`-guarded), `docs/OPERATIONS.md` | feat/maintenance-crons |
| Dark mode (light/dark/system theme) | P2 | `tailwind.config.ts` (`darkMode:"class"` + `canvas`/`surface`/`surface-2`/`ink`/`muted`/`faint`/`line` tokens), `app/globals.css` (CSS-var tokens + `.dark`), `lib/theme.ts`, `components/ThemeProvider.tsx`, `app/layout.tsx` (cookie-read + inline anti-flash script), `app/api/settings/theme/route.ts` (PATCH + `hearth-theme` cookie), `components/ThemePrefs.tsx`, `(app)/settings/appearance`, `components/SettingsMenu.tsx`, `app/auth/callback/route.ts` (seed cookie from DB), `profiles.theme`, `drizzle/0008_*.sql` | feat/dark-mode |
